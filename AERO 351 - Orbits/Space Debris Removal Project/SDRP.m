...
[timeNew,Orbit1] = ode45(@twobodymotion,timespan,state,options,mu);
...
[~,~,~,nuL12,~,~,~,p,t,~,Ra] = rv2coes(RL1,VL1,mu,Rearth);
...
state2 = [R22, V22];
tspan2 = [0, tA2];
[timeNew,Orbit2A] = ode45(@twobodymotion,tspan2,state2,options,mu);

R2A = [Orbit2A(end,1),Orbit2A(end,2),Orbit2A(end,3)]; %R&V at apogee orb 2
V2A = [Orbit2A(end,4),Orbit2A(end,5),Orbit2A(end,6)];

[R12_3,V12_3] = keplerian2ijk(aL2*1000,eccL2,incL2,RAANL2,ArgPL2,nuL2);
R12_3 = R12_3./1000;
V12_3 = V12_3./1000;
...
figure('Name', 'Orbit Trajectory 2D');
plot(0, 0, 'g*', 'MarkerSize', 10); % Earth at the origin
hold on;
%plot(Orbit1(:, 1), Orbit1(:, 2), 'b', 'LineWidth', 1.5);
plot(Trans12_1(:, 1), Trans12_1(:, 2), 'r', 'LineWidth', 1.5);
%plot(Orbit2(:, 1), Orbit2(:, 2), 'g', 'LineWidth', 1.5);
plot(Trans12_1(end,1),Trans12_1(end,2), 'r*', 'MarkerSize', 10);
plot(Orbit1(end,1),Orbit1(end,2), 'w*', 'MarkerSize', 10); 
plot(Orbit2(end,1),Orbit2(end,2), 'g*', 'MarkerSize', 10);
%plot(Orbit1A(end,1),Orbit1A(end,2), 'y*', 'MarkerSize', 10);
plot(Trans12_2(:,1),Trans12_2(:,2), 'r', 'MarkerSize', 10);
plot(R2(1),R2(2), 'w*', 'MarkerSize', 10);
plot(Orbit2A(end,1),Orbit2A(end,2), 'c*', 'MarkerSize', 10);
ylabel('Y (km)');
zlabel('Z (km)');
grid on;
