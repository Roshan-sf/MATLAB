...
y = 1;%Used in the while loops as a place holder for column number while the loops search for min number
i = 0;%Represents the lowest number in row 1
j = 0;%Represents the lowest number in row 2
k = 0;%Represents the lowest number in row 3
row1Col = 0;%Represents the column number which holds the lowest number in row 1
row2Col = 0;%Represents the column number which holds the lowest number in row 2
row3Col = 0;%Represents the column number which holds the lowest number in row 3
a = 0;%is a 3x1 matrix containing all 3 minimum values from each row


for row = 1:3 %two for loops creating a 3x4 matrix
    for col = 1:4
        randNum = rand;
...
a = min(array_loop,[],2); %the number 2 searches whole row, if switched with a 1 it searches each column
%disp(a)
...
while row1Col == 0 %Continuously searches each column in row 1 for the lowest number
   if array_loop(1,y) ~= i
    y = y + 1;
...
while row2Col == 0 %Continuously searches each column in row 2 for the lowest number
   if array_loop(2,y) ~= j
    y = y + 1;
...
while row3Col == 0 %Continuously searches each column in row 3 for the lowest number
   if array_loop(3,y) ~= k
    y = y + 1;
...
Prius.color = 'white'; %Creating a structure for a prius containting all pertinant information
Prius.year = 2013;
Prius.make = 'toyota';
...
Civic.color = 'white'; %Creating a structure for a civic containting all pertinant information
Civic.year = 2015;
Civic.make = 'honda';
Civic.body = 'sedan';
Civic.power = 143;
...
priRatio = Prius.power / Prius.weight; %Determinng the power to weight ratios
civRatio = Civic.power / Civic.weight;

if priRatio < civRatio %Displaying each ratio
...
xlabel('Angles [rad] from 0 to 2pi'); %xlabel
ylabel('Sin(x)') %ylabel
title('Graph of Sin(x)') %title
