...
disp('Part 1:')
disp(['g vector in meters: ', num2str(g'.*1000)])
disp(['gnorm: ', num2str(g_norm'.*1000)])
...
for i = 1:length(altVec)
    ...
    g_errors(i) = loc_error.*1000; %convrt km to m
...
function [g, g_norm, T_g, monteCarloData] = geo_monte_sim(r_0, v_0, phat_0, T_0, JD, error, target, Re, n, c)
    ...
    for i = 1:n
        ...
        d_r_vel = vErr * lsdErr * randn(3,1);  % Random velocity-based shift
        d_r_radial = (vErr_r * lsdErr * randn()) * (r_0 / norm(r_0));  % Radial error
